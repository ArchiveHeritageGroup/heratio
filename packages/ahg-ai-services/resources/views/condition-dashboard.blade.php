...
@section('title', 'Condition Dashboard')
@section('body-class', 'browse ai')

@section('content')
  <div class="multiline-header d-flex align-items-center mb-3">
    <i class="fas fa-3x fa-robot me-3" aria-hidden="true"></i>
    <div class="d-flex flex-column"><h1 class="mb-0">Condition Dashboard</h1></div>
  </div>
  ...
  @if(isset($rows) && count($rows))
    <div class="table-responsive"><table class="table table-bordered table-hover mb-0">
      ...
      <tbody>@foreach($rows as $i => $row)<tr><td>{{ $i + 1 }}</td><td>{{ $row->name ?? $row->title ?? '-' }}</td><td><span class="badge bg-secondary">{{ $row->status ?? '-' }}</span></td><td>{{ $row->created_at ?? '-' }}</td><td><a href="#" class="btn btn-sm btn-outline-secondary"><i class="fas fa-eye"></i></a></td></tr>@endforeach</tbody>
    </table></div>
