...
The ahgContactPlugin provides extended contact information management for Heratio authority records (persons, organizations) and repositories. It extends the core Heratio contact_information table with additional fields for professional contact details while maintaining full backward compatibility.
...
```
+------------------------------------+
|        contact_information         |   (Heratio Core Table)
+------------------------------------+
| PK id INT                          |
|    actor_id INT                    |------+
...
```sql
-- Core Heratio Table (DO NOT MODIFY)
-- contact_information
