...
@section('content')
  ...
  @include('ahg-ric::_view-switch', ['standard' => 'ISAAR(CPF)', 'entityType' => 'actor', 'objectId' => $actor->id])
  ...
  @php
    ...
    $canDelete = \AhgCore\Services\AclService::check($actor, 'delete');
  ...
  <section class="actions">
    <ul class="nav gap-2">
      ...
      @if($canUpdate)
        ...
        @if(empty($digitalObjects['master']))
          <li>
            <a class="btn atom-btn-outline-light" href="{{ route('actor.digitalobject.add', $actor->slug) }}">
              <i class="fas fa-image me-1"></i>{{ __('Link digital object') }}
            </a>
          </li>
        @endif
      @endif
