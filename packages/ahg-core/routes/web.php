...
// heratio#1244 (WARC web-archiving slice) - the web-archive ADMIN SURFACE now lives in ONE place:
// the ahg-scan package (AhgScan\Controllers\WebArchiveController, routes/web-archive.php, route
// names web-archive.*). That single surface offers BOTH capture modes (archive a submitted URL,
// AND snapshot a published record's own page) and is backed by the reusable ENGINES that stay
// here in ahg-core: WarcCaptureService (SSRF-guarded capture for both modes + same-host
// subresources) and WarcReplayService (the length-delimited WARC 1.1 parser + replay model). The
// duplicate ahg-core WebArchiveController + its routes were removed in the #1244 merge so there is
// exactly ONE controller registering the web-archive.* names (no route collision). The single
// warc_capture table + the warc_capture_status dropdown are still installed from this package's
// service provider; the .warc bytes live on disk under config('heratio.storage_path').'/web-archive'.
