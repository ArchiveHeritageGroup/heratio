...
@section('content')
  ...
  <div class="card-body"><form method="POST" action="{{ $formAction ?? '#' }}">@csrf
    <div class="mb-3"><label class="form-label">Backup Archive <span class="badge bg-secondary ms-1">Required</span></label><input type="file" class="form-control" name="backup_file" accept=".zip,.sql,.gz" required></div>
