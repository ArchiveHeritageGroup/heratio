...
High-performance replacement for base Heratio's taxonomy browse (`/taxonomy/:id`) and term browse (`/term/:slug`). Eliminates N+1 query patterns by using batch count queries and direct Elasticsearch HTTP requests instead of Elastica ORM objects.
...
Both routes are prepended via `routing.load_configuration` event, overriding base Heratio's default `/:module/:action` catch-all.
...
No custom tables. Uses existing Heratio tables (read-only):
