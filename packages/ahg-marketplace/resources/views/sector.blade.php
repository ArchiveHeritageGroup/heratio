...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('.fav-toggle').forEach(function (btn) {
    btn.addEventListener('click', function (e) {
      ...
      var id = btn.getAttribute('data-listing-id'); if (!id) return;
      var icon = btn.querySelector('i'); btn.disabled = true;
      fetch('/marketplace/api/' + id + '/favourite', {
        ...
        .then(function (d) {
          if (d && d.favourited) { icon.classList.remove('far'); icon.classList.add('fas'); btn.title = 'Remove from favourites'; }
          else { icon.classList.remove('fas'); icon.classList.add('far'); btn.title = 'Add to favourites'; }
        })
