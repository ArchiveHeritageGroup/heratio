...
class RadManageController extends Controller
{
    ...
    public function persist(int $ioId, Request $request): void
    {
        $culture = app()->getLocale();
            ...
            foreach (self::RAD_PROPERTIES as $propName) {
                $this->saveProperty($ioId, $propName, $request->input($propName), $culture);
            }
            ...
            // Languages/scripts of description
            $this->saveSerializedProperty($ioId, 'languageOfDescription', $request->input('languagesOfDescription', []), $culture);
            $this->saveSerializedProperty($ioId, 'scriptOfDescription', $request->input('scriptsOfDescription', []), $culture);

            // Events (dates with type, actor, place)
            if ($request->has('_eventsIncluded')) {
                $eventData = $request->input('events', []);
                $existingEventIds = DB::table('event')->where('object_id', $ioId)->pluck('id');
                if ($existingEventIds->isNotEmpty()) {
                    DB::table('event_i18n')->whereIn('id', $existingEventIds)->delete();
                    DB::table('event')->where('object_id', $ioId)->delete();
    ...
    /** Render the RAD field partial for the dynamic-standard swap (#1425). */
    public function fieldsPartial(Request $request, ?string $slug = null)
    {
        ...
        $data = ['io' => null, 'subjects' => collect(), 'places' => collect(), 'genres' => collect(),
                 ...
                 'materialTypes' => collect(), 'alternativeIdentifiers' => collect(), 'publicationStatusId' => null,
                 'materialLanguages' => collect(), 'materialScripts' => collect(), 'languagesOfDescription' => collect(),
                 'scriptsOfDescription' => collect(), 'radProperties' => collect(), 'relatedMaterialDescriptions' => collect()];
        if ($slug) {
            $io = DB::table('information_object')
                ->join('information_object_i18n', 'information_object.id', '=', 'information_object_i18n.id')
                ->join('slug', 'information_object.id', '=', 'slug.object_id')
        ...
        return view('rad-manage::_fields', array_merge($data, $dropdowns));
    }
