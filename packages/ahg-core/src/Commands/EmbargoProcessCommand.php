    ...
    public function handle(EmbargoService $svc): int
    {
        ...
        if (! $notifyOnly) {
            foreach ($expired as $e) {
                if ($dry) {
                    $this->line("  would lift embargo id={$e->id} object={$e->object_id} (end_date={$e->end_date})");
                    $lifted++;

                    continue;
                }
                if ($svc->liftEmbargo((int) $e->id, (int) $this->option('system-user-id'), 'auto-lifted at expiry')) {
                    $lifted++;
                }
            }
        }
        ...
        if (! $liftOnly) {
            ...
            foreach ($expiring as $e) {
                if (! ($e->notify_on_expiry ?? 1)) {
                    continue;
                }
                if ($dry) {
                    $this->line("  would notify embargo id={$e->id}");
                    $notified++;
