...
@php
    ...
    $href = \Illuminate\Support\Facades\Route::has('registry.groupView')
        ...
        : '#';
    $loc = trim(implode(', ', array_filter([$item->city ?? '', $item->country ?? ''])));
@endphp
<div class="col">
    <div class="card h-100">
        <div class="card-body">
            <div class="d-flex align-items-start mb-2">
                ...
                <div class="min-width-0">
                    <h6 class="card-title mb-1">
                        <a href="{{ $href }}" class="text-decoration-none stretched-link">{{ $item->name ?? '' }}</a>
            ...
            <div class="small text-muted mb-2">
                <i class="fas fa-users me-1"></i>{{ (int) ($item->member_count ?? 0) }} {{ __('members') }}
            </div>

            @if (! empty($item->meeting_frequency))
                <div class="small text-muted mb-2">
                    <i class="fas fa-calendar me-1"></i>{{ ucfirst(str_replace('_', ' ', $item->meeting_frequency)) }}
                </div>
            @endif

            @if (! empty($item->description))
                <p class="card-text small text-muted mb-0">{{ \Illuminate\Support\Str::limit(strip_tags($item->description), 100) }}</p>
            @endif
        </div>
    </div>
</div>
