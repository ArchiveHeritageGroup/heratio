...
<div class="card-body"><div class="row">
  ...
  <div class="col-md-6 mb-3"><label for="parent_id" class="form-label">Parent Collection <span class="badge bg-secondary ms-1">{{ __('Optional') }}</span></label><select name="parent_id" id="parent_id" class="form-select"><option value="">-- None (top level) --</option>@foreach($allCollections ?? [] as $c)<option value="{{ $c->id }}" {{ ($parentId ?? '') == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>@endforeach</select></div>
  <div class="col-md-6 mb-3"><div class="form-check"><input class="form-check-input" type="checkbox" name="is_public" value="1" id="is_public" checked><label class="form-check-label" for="is_public">Public <span class="badge bg-secondary ms-1">{{ __('Optional') }}</span></label></div></div>
</div></div></div>
<section class="actions mb-3 nav gap-2" style="background-color:#495057;border-radius:.375rem;padding:1rem;"><a href="{{ route('iiif-collection.index') }}" class="btn atom-btn-outline-light">Cancel</a><button type="submit" class="btn atom-btn-outline-light">{{ __('Create Collection') }}</button></section>
</form>
@endsection
