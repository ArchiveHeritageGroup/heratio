  ...
  <div class="container">
    ...
    <div class="row g-4">
      ...
      <div class="col-md-6 col-lg-4">
        <a href="{{ route('heritage.timeline', ['period_id' => $period['id']]) }}" class="card h-100 text-decoration-none" @if($period['background_color']) style="border-left:4px solid {{ $period['background_color'] }}" @endif>
    ...
    <div class="d-flex flex-wrap gap-2 mb-4">
      ...
      <a href="{{ route('heritage.timeline', ['period_id'=>$p['id']]) }}" class="btn btn-sm {{ $p['id']==$currentPeriod['id'] ? 'atom-btn-secondary' : 'atom-btn-white' }}">{{ $p['short_name'] ?? $p['name'] }}</a>
      @endforeach
    </div>
    @endif
