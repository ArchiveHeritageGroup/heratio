...
class IntegrityReportCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        if ($this->option('auditor-pack')) {
            return $this->auditorPack((string) $this->option('auditor-pack'));
        }
    ...
    protected function renderText(array $r): void
    {
        ...
        foreach ($r['runs'] as $s => $row) {
            $this->line(sprintf('  %-12s n=%-5d verified=%-7d failed=%d',
                $s, $row->n ?? 0, $row->verified ?? 0, $row->failed ?? 0));
        }
        $this->info("\n=== fixity check results ===");
        foreach ($r['fixity_results'] as $k => $n) {
            $this->line(sprintf('  %-12s %d', $k, $n));
        }
        $this->info("\n=== alerts by severity ===");
        foreach ($r['alerts'] as $k => $n) {
            $this->line(sprintf('  %-12s %d', $k, $n));
        }
    }
    ...
    protected function writeCsv(array $r, string $path): void
    {
        ...
        foreach (['runs' => 'integrity_runs', 'fixity_results' => 'fixity', 'alerts' => 'alerts'] as $k => $label) {
            foreach ($r[$k] as $kk => $v) {
                fputcsv($h, [$label, $kk, is_object($v) ? json_encode($v) : $v]);
            }
        }
        fclose($h);
    }
    ...
    protected function deadLetter(): int
    {
        if (! Schema::hasTable('integrity_dead_letter')) {
            $this->warn('integrity_dead_letter not present.');
