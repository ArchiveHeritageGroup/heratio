...
This folder consolidates everything in one place:
...
- The **workbook** is the spine. Every principle maps to a control, every control maps
  to a tick-sheet line, and every tick-sheet line maps to a regulatory obligation via
  the **compliance control catalog**.
- The **diagrams** are the shared visual layer for both the workbook and the articles.
- The **articles** are the narrative spokes; the workbook is the hub they point to.

## Working tick-sheets in this workbook

The workbook turns the framework's checklists into fillable `- [ ]` lists you copy per
pilot: the minimum-viable-deployment checklist, the vendor / procurement checklist, the
acceptance tests, the regime-to-control mapping checklist, and the quick compliance
checklist. Several supporting articles also carry their own templates and tick-sheets
(see `workbook.md` → "Supporting reading").

## Status

- Content is drawn from the framework and the published articles. It contains **no
  invented metrics or organisation names** - the pilot numbers and names are placeholders
  to replace with your own signed pilot outputs.
- Jurisdiction-neutral throughout. Country regimes (GDPR, FOI/access-to-information,
  health-data, etc.) appear only as interchangeable examples, never as the default.
