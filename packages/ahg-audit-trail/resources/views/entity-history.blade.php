...
@section('title', 'Entity History')
@section('body-class', 'browse audit')
@section('content')
  <div class="multiline-header d-flex align-items-center mb-3">
    <i class="fas fa-3x fa-shield-alt me-3" aria-hidden="true"></i>
    <div class="d-flex flex-column"><h1 class="mb-0">{{ __('Entity History') }}</h1></div>
  </div>
  @if(isset($rows) && count($rows))
    <div class="table-responsive"><table class="table table-bordered table-hover mb-0">
      <thead><tr><th>#</th><th>{{ __('Details') }}</th><th>{{ __('User') }}</th><th>{{ __('Date') }}</th><th>{{ __('Actions') }}</th></tr></thead>
  ...
  @endif
  <div class="mt-3"><a href="{{ route('audit.browse') }}" class="btn atom-btn-white"><i class="fas fa-arrow-left me-1"></i> {{ __('Back to Audit Trail') }}</a></div>
@endsection
