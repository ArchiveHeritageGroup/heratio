...
class DoiDeactivateCommand extends Command
{
    ...
    public function handle(DoiService $svc): int
    {
        if ($this->option('list-deleted')) {
            ...
            foreach ($rows as $r) {
                $this->line(sprintf('  %s  oid=%s  at=%s', $r->doi, $r->information_object_id, $r->last_sync_at));
            }
        ...
        if ($d = $this->option('doi')) {
            if ($this->option('dry-run')) {
                $this->info("would tombstone {$d} ({$reason})");
        ...
        if ($oid = $this->option('object-id')) {
            if ($this->option('dry-run')) {
                $this->info("would tombstone DOI for oid={$oid}");
