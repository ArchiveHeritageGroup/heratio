...
The ahgIiifPlugin provides comprehensive IIIF (International Image Interoperability Framework) capabilities for Heratio, including manifest generation, deep zoom viewing, media streaming with on-the-fly transcoding, annotation support, collection management, and authentication (IIIF Auth API 1.0). The plugin supports images, PDFs, multi-page TIFFs, 3D models, and audio/video content.
...
```
information_object (Heratio core)
    │
...
|---------|-------------|----------|
| `login` | Requires Heratio authentication | Registered users only |
