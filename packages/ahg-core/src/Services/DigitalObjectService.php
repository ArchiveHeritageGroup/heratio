...
class DigitalObjectService
{
    ...
    /**
     * Find a web-optimized PDF derivative for an entity, if one has been
     * generated by `ahg:optimize-pdfs`. This is a usage_id=141 (reference)
     * child whose mime_type is application/pdf - distinct from the generic
     * PNG icon that older code registered as the reference for PDFs.
     *
     * @return object|null  the digital_object row, or null when none exists
     */
    public static function getWebPdfReference(int $objectId)
    {
        if (! $objectId) {
            return null;
        }
    ...
    /**
     * Web URL of the optimized PDF derivative for an entity, or null.
     * The document viewer prefers this over the (potentially huge) master so
     * big scans load page-1-fast; the master stays available for download.
     */
    public static function getWebPdfUrl(int $objectId): ?string
    {
        $ref = self::getWebPdfReference($objectId);

        return $ref ? self::getUrl($ref) : null;
    }

    /**
     * Resolve the on-disk absolute path for a digital_object row. The web-facing
     * `path` starts with /uploads/r/ (or /uploads/) which is stripped before
     * joining with config('heratio.uploads_path'); several legacy layouts are
     * tried so this works across instances. Returns null when nothing resolves.
     */
    public static function resolveDiskPath($digitalObject): ?string
    {
        if (! $digitalObject || empty($digitalObject->name)) {
            return null;
        }
