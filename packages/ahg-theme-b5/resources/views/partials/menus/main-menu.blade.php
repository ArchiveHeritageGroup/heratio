...
<li class="nav-item dropdown d-flex flex-column">
  <a class="nav-link dropdown-toggle d-flex align-items-center p-0" href="#" id="add-menu" role="button" data-bs-toggle="dropdown" aria-expanded="false">
    ...
    <span class="d-lg-none mx-1" aria-hidden="true">{{ __('Add') }}</span>
    <span class="visually-hidden">{{ __('Add') }}</span>
  </a>
    ...
    @if($infoObjectItem)
      <li>
        <a class="dropdown-item" href="{{ MenuService::resolvePath($infoObjectItem->path) }}">
          <i class="{{ $iconMap[$infoObjectItem->name] ?? 'fas fa-plus' }} me-2"></i>{{ __('Archival description') }}
        </a>
      </li>
    @endif
    @if($hasMuseum)<li><a class="dropdown-item" href="{{ url('/museum/add') }}"><i class="fas fa-university fa-fw me-2"></i>{{ __('Museum object') }}</a></li>@endif
    @if($hasGallery)<li><a class="dropdown-item" href="{{ url('/gallery/add') }}"><i class="fas fa-images fa-fw me-2"></i>{{ __('Gallery item') }}</a></li>@endif
    @if($hasLibrary)<li><a class="dropdown-item" href="{{ url('/library/add') }}"><i class="fas fa-book fa-fw me-2"></i>{{ __('Library item') }}</a></li>@endif
    @if($hasDam)<li><a class="dropdown-item" href="{{ url('/dam/create') }}"><i class="fas fa-photo-video fa-fw me-2"></i>{{ __('Photo/DAM asset') }}</a></li>@endif
...
<li class="nav-item dropdown d-flex flex-column">
  <a class="nav-link dropdown-toggle d-flex align-items-center p-0" href="#" id="manage-menu" role="button" data-bs-toggle="dropdown" aria-expanded="false">
    ...
    <span class="d-lg-none mx-1" aria-hidden="true">{{ __('Manage') }}</span>
    <span class="visually-hidden">{{ __('Manage') }}</span>
  </a>
  <ul class="dropdown-menu dropdown-menu-end mb-2" aria-labelledby="manage-menu">
    ...
    <li>
      <a class="dropdown-item" href="{{ route('reports.dashboard') }}">
        <i class="fas fa-tachometer-alt fa-fw me-2"></i>{{ __('Central Dashboards') }}
      </a>
    </li>
  </ul>
</li>
...
<li class="nav-item dropdown d-flex flex-column">
  <a class="nav-link dropdown-toggle d-flex align-items-center p-0" href="#" id="import-menu" role="button" data-bs-toggle="dropdown" aria-expanded="false">
    ...
    <span class="d-lg-none mx-1" aria-hidden="true">{{ __('Import') }}</span>
    <span class="visually-hidden">{{ __('Import') }}</span>
  </a>
...
<li class="nav-item dropdown d-flex flex-column">
  <a class="nav-link dropdown-toggle d-flex align-items-center p-0" href="#" id="admin-menu" role="button" data-bs-toggle="dropdown" aria-expanded="false">
    ...
    <span class="d-lg-none mx-1" aria-hidden="true">{{ __('Admin') }}</span>
