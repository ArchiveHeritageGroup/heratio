...
Framework migration of base Heratio's archival institution browse (`/repository/browse`). Replaces Elastica ORM with direct ES HTTP (curl) queries and Laravel Query Builder for aggregation population and advanced filter data. Fully compatible with the existing theme templates and DisplayModeService.
...
Prepended via `routing.load_configuration` event, overriding base Heratio's default route.
...
Six facets matching base Heratio's `RepositoryBrowseAction::$AGGS`:
...
No custom `app.yml` settings. Uses standard Heratio/framework settings:
...
No custom tables. Uses existing Heratio tables (read-only via Laravel QB):
