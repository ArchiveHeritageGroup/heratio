...
@section('content')
  <div class="multiline-header d-flex align-items-center mb-3">
    ...
    <div class="d-flex flex-column"><h1 class="mb-0">{{ __('Authority Identifiers') }}</h1></div>
  </div>
  @if(isset($rows) && count($rows))
    <div class="table-responsive"><table class="table table-bordered table-hover mb-0">
      <thead><tr><th>#</th><th>{{ __('Authority') }}</th><th>{{ __('Type') }}</th><th>{{ __('Value') }}</th><th>{{ __('Source') }}</th></tr></thead>
