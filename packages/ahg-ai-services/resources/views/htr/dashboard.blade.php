...
@if($health)
<div class="card mb-4">
  <div class="card-header" style="background: var(--ahg-primary); color: white;"><i class="fas fa-server me-2"></i>{{ __('Service Status') }}</div>
  <div class="card-body">
    <div class="row">
      <div class="col-md-3"><strong>{{ __('GPU:') }}</strong> {{ $health['gpu_name'] ?? 'N/A' }}</div>
      <div class="col-md-3"><strong>{{ __('VRAM:') }}</strong> {{ $health['vram'] ?? 'N/A' }}</div>
      <div class="col-md-3"><strong>{{ __('Model:') }}</strong> <span class="badge bg-success">{{ !empty($health['model_loaded']) ? 'Loaded' : 'Not loaded' }}</span></div>
      <div class="col-md-3"><strong>{{ __('Version:') }}</strong> {{ $health['version'] ?? '1.0.0' }}</div>
    </div>
  </div>
</div>
@else
<div class="alert alert-danger"><i class="fas fa-exclamation-triangle me-2"></i>{{ __('HTR service is offline.') }}</div>
@endif
