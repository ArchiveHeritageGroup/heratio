    ...
    <div class="col-md-9">
        ...
        <section class="mb-3">
            ...
            <table class="table table-sm">
                <tr><th style="width:180px">{{ __('Type') }}</th><td><span class="badge bg-info">{{ $entity->type_id ?? 'Not set' }}</span></td></tr>
                <tr><th>{{ __('Start Date') }}</th><td>{{ $entity->start_date ?? '' }}</td></tr>
                <tr><th>{{ __('End Date') }}</th><td>{{ $entity->end_date ?? '' }}</td></tr>
                <tr><th>{{ __('Date Display') }}</th><td>{{ $entity->date_display ?? '' }}</td></tr>
                @if($entity->place_name)<tr><th>{{ __('Place') }}</th><td>{{ $entity->place_name }}</td></tr>@endif
