...
<div class="container-fluid mt-3">
    ...
    <div class="card-body">
      <form method="POST" action="{{ route('security-clearance.trace-watermark-result') }}">
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="mb-3">
              <label class="form-label">{{ __('Watermark Code') }}</label>
              <input type="text" name="code" class="form-control" value="{{ old('code', request('code', '')) }}" placeholder="{{ __('e.g. WM-2024-ABC123') }}" required>
            </div>
          </div>
  ...
  <div class="card">
    <div class="card-header"><h5 class="mb-0">{{ __('Trace Result') }}</h5></div>
    <div class="card-body">
      @if($traceResult)
        <table class="table">
          <tr><th>{{ __('User') }}</th><td>{{ e($traceResult->username ?? '') }}</td></tr>
          <tr><th>{{ __('Email') }}</th><td>{{ e($traceResult->email ?? '') }}</td></tr>
          <tr><th>{{ __('Object') }}</th><td>{{ e($traceResult->object_title ?? '') }}</td></tr>
          <tr><th>{{ __('Downloaded At') }}</th><td>{{ $traceResult->downloaded_at ?? '' }}</td></tr>
          <tr><th>{{ __('IP Address') }}</th><td>{{ e($traceResult->ip_address ?? '') }}</td></tr>
