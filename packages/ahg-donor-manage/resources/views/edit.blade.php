...
@section('content')
  ...
  <form method="POST" action="{{ $donor ? route('donor.update', $donor->slug) : route('donor.store') }}">
    ...
    <ul class="actions mb-3 nav gap-2">
      @if($donor)
        <li><a href="{{ route('donor.show', $donor->slug) }}" class="btn btn-outline-secondary">Cancel</a></li>
        <li><input class="btn btn-outline-success" type="submit" value="Save"></li>
      @else
        <li><a href="{{ route('donor.browse') }}" class="btn btn-outline-secondary">Cancel</a></li>
