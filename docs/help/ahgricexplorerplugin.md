...
The ahgRicExplorerPlugin implements the ICA's Records in Contexts Ontology (RiC-O) for archival descriptions, providing extraction, storage, visualization, and semantic search capabilities. It transforms Heratio's relational data into RDF linked data and synchronizes it with an Apache Jena Fuseki triplestore.
...
### Heratio to RiC-O Entity Mapping

| Heratio Entity | RiC-O Class | Notes |
|-------------|-------------|-------|
...
The listener hooks into Heratio's event dispatcher to capture entity changes:
...
The main extraction tool converts Heratio MySQL data to RiC-O JSON-LD:
...
```json
{
  ...
  "_metadata": {
    ...
    "source": "Heratio instance: atom-psis",
    "records_count": 150,
    "agents_count": 25,
    "relations_count": 340
  }
