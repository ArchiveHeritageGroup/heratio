...
Universal metadata extraction plugin that automatically extracts embedded metadata from uploaded digital objects using multiple extraction methods (native PHP, ExifTool, FFprobe). Supports images, PDFs, Office documents, video, and audio files with automatic field mapping to Heratio descriptive fields.
...
The plugin uses the standard Heratio settings table with scope `metadata_extraction`:
