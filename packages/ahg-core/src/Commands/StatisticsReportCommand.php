...
class StatisticsReportCommand extends Command
{
    ...
    protected function format(array $rows, string $format): string
    {
        ...
        if ($format === 'csv') {
            ...
            foreach ($rows as $r) {
                $lines[] = implode(',', array_map(fn ($v) => is_scalar($v) ? (string) $v : json_encode($v), $r));
            }
        ...
        foreach ($rows as $r) {
            foreach ($r as $k => $v) {
                $widths[array_search($k, $headers)] = max($widths[array_search($k, $headers)], strlen((string) $v));
            }
        }
