...
/**
 * PlaceService - the read-only data layer behind the public "Browse by place"
 * discovery surface (the geography slice; sibling of the "Explore by theme"
 * subject slice in ThemeService).
 *
 * A place is a geographic access point in the catalogue: the place terms
 * (taxonomy 42) that the most PUBLISHED records are ABOUT are the collection's
 * busiest places. This service surfaces them as "ways into the collection by
 * geography":
 *
 *   - topPlaces()        - the busiest place terms by published-record count
 ...
 *   - placeList()        - the same compact list for the machine-readable .json
 *                          twin (id, label, count only).
 *   - place()            - one place: its label + scope note + a paginated,
 *                          bounded list of the published records about it.
 *
 ...
 * place names come entirely from the data; no geography is hardcoded and there is
 * no country default.
 *
...
class PlaceService
{
    /** Place taxonomy id - the geographic access points of the collection. */
    ...
    /** Hard ceiling on how many places the landing aggregate ever returns. */
    public const MAX_PLACES = 120;

    /** Default places shown on the landing cloud. */
    public const DEFAULT_PLACES = 80;

    /** Default + ceiling for the per-place paginated record list. */
    ...
    public function available(): bool
    {
        try {
            ...
            Log::info('[places] table probe failed: '.$e->getMessage());
    ...
    /**
     * The busiest place terms by PUBLISHED-record count - the collection's
     * geographic "ways in". One cheap bounded GROUP BY aggregate over
     * object_term_relation joined to term (taxonomy 42) and the publication-status
     * table, ordered by count, capped at MAX_PLACES. Read-only; never throws -
     * degrades to [].
     *
     * @param  int  $limit  how many places to return (clamped 1..MAX_PLACES)
     * @return array<int,array{term_id:int,label:string,record_count:int,url:string,browse_url:string}>
     */
    public function topPlaces(int $limit = self::DEFAULT_PLACES): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $out[] = [
                'term_id' => $termId,
                'label' => $labels[$termId] ?? (__('Place').' #'.$termId),
                'record_count' => (int) $row->record_count,
                'url' => $this->placeUrl($termId),
    ...
    /**
     * A compact place list for the machine-readable .json twin: id, label and
     ...
     */
    public function placeList(int $limit = self::MAX_PLACES): array
    {
        $rows = $this->aggregate($limit);
        if (empty($rows)) {
            return [];
    ...
    /**
     * The shared bounded aggregate behind topPlaces() and placeList(): group the
     * published object<->place-term links by term, count distinct records, keep the
    ...
    protected function aggregate(int $limit): array
    {
        ...
        $limit = max(1, min($limit, self::MAX_PLACES));
        ...
        try {
            ...
            Log::info('[places] aggregate failed: '.$e->getMessage());
    ...
    /**
     * One place in full: its label + scope note, its total published-record count,
     * and a single bounded page of the published records about it. The record list
     * is paginated (page + per-page offset) and capped at MAX_PER_PAGE so a busy
     * place can never run an unbounded query. Returns null when the term is
     * missing, is not a place term, or has no published records. Read-only; never
     * throws.
     *
     * @return array<string,mixed>|null
     */
    public function place(int $termId, int $page = 1, int $perPage = self::PER_PAGE): ?array
    {
        ...
        // The term must exist AND be a place term (taxonomy 42). A subject or genre
        // id would not be a "place" and is rejected here.
        try {
            $isPlace = DB::table('term')
                ->where('id', $termId)
                ->where('taxonomy_id', self::PLACE_TAXONOMY_ID)
                ->exists();
        } catch (\Throwable $e) {
            Log::info('[places] term probe failed for '.$termId.': '.$e->getMessage());
        ...
        $label = $this->termLabels([$termId], $culture)[$termId] ?? (__('Place').' #'.$termId);
        $note = $this->termNote($termId, $culture);

        // Total published records about this place (cheap COUNT DISTINCT).
        try {
            ...
            Log::info('[places] place count failed for '.$termId.': '.$e->getMessage());
            $total = 0;
        }
    ...
    /**
     * A single bounded page of PUBLISHED records about a place term, ordered by
    ...
    protected function publishedRecordsPage(int $termId, int $page, int $perPage, string $culture): array
    {
        try {
            ...
            Log::info('[places] records page failed for '.$termId.': '.$e->getMessage());
    ...
    protected function hydrateRecords(array $ids, string $culture): array
    {
        ...
        try {
            ...
            Log::info('[places] title hydrate failed: '.$e->getMessage());
        }
        ...
        try {
            ...
            Log::info('[places] slug hydrate failed: '.$e->getMessage());
        }
    ...
    protected function termLabels(array $termIds, string $culture): array
    {
        ...
        try {
            ...
            Log::info('[places] term labels failed: '.$e->getMessage());
        }
    ...
    protected function termNote(int $termId, string $culture): ?string
    {
        try {
            ...
            Log::info('[places] term note failed for '.$termId.': '.$e->getMessage());
    ...
    /**
     * The public per-place detail URL for one place term. url()-relative, never a
     * hardcoded host.
     */
    public function placeUrl(int $termId): string
    {
        return url('/places/'.$termId);
    }

    /**
     * The GLAM-browse deep link for one place - reuses the single canonical browse
     * page (ahg-display) with its `place=<term_id>` filter, so "browse all records
     * about this place" lands in the same place as the facet. url()-relative,
     * never a hardcoded host.
     */
    public function browseUrl(int $termId): string
    {
        return url('/glam/browse?place='.$termId);
    }
