  ...
  <form method="get" action="{{ route('strongroom.browse') }}" class="mb-3" role="search">
    <div class="input-group" style="max-width: 32rem;">
      <input type="search" name="q" value="{{ $search }}" class="form-control"
             placeholder="{{ __('Search strongrooms') }}">
        ...
        <tbody>
          @foreach($rooms as $room)
            ...
            <tr>
              ...
              <td>
                @if($pct !== null)
                  <div class="progress" role="progressbar" aria-valuenow="{{ $pct }}" aria-valuemin="0" aria-valuemax="100" style="height: 1rem;">
  ...
  @else
    <p class="text-muted">
      ...
      @else
        ...
        @auth <a href="{{ route('strongroom.create') }}">{{ __('Add the first one.') }}</a> @endauth
      @endif
    </p>
  @endif
@endsection
