...
<div class="card"><div class="card-body">
    <form method="POST">@csrf
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-3"><label class="form-label">Format <span class="badge bg-danger ms-1">{{ __('Required') }}</span></label><select name="format" class="form-select"><option value="pdf">PDF</option><option value="tiff">TIFF</option><option value="jpeg">JPEG</option><option value="paper">{{ __('Paper') }}</option></select></div>
            <div class="col-md-3"><label class="form-label">Quantity <span class="badge bg-danger ms-1">{{ __('Required') }}</span></label><input type="number" name="quantity" class="form-control" value="1" min="1"></div>
        </div>
        <div class="mb-3"><label class="form-label">Purpose <span class="badge bg-danger ms-1">{{ __('Required') }}</span></label><select name="purpose" class="form-select"><option value="personal_research">{{ __('Personal Research') }}</option><option value="publication">{{ __('Publication') }}</option><option value="exhibition">{{ __('Exhibition') }}</option><option value="legal">{{ __('Legal') }}</option><option value="other">{{ __('Other') }}</option></select></div>
        <div class="mb-3"><label class="form-label">Special Instructions <span class="badge bg-danger ms-1">{{ __('Required') }}</span></label><textarea name="notes" class="form-control" rows="3" placeholder="{{ __('Page ranges, specific sections, quality requirements...') }}"></textarea></div>
