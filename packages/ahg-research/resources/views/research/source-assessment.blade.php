...
<div class="card mb-4"><div class="card-header" style="background:var(--ahg-primary);color:#fff">Assessment Details</div><div class="card-body">
    <form method="POST">@csrf <input type="hidden" name="object_id" value="{{ $objectId ?? 0 }}">
        <div class="row mb-3">
            <div class="col-md-6"><label class="form-label">Source Type <span class="text-danger">*</span></label><select name="source_type" class="form-select" required>
            ...
            <div class="col-md-6"><label class="form-label">Completeness</label><select name="completeness" class="form-select">
        ...
        <div class="mb-3"><label class="form-label">Provenance</label><textarea name="provenance" class="form-control" rows="2">{{ e($assessment->provenance ?? '') }}</textarea></div>
        <div class="mb-3"><label class="form-label">Authenticity Notes</label><textarea name="authenticity_notes" class="form-control" rows="2">{{ e($assessment->authenticity_notes ?? '') }}</textarea></div>
        <div class="row mb-3">
            <div class="col-md-6"><label class="form-label">Reliability (1-5)</label><input type="number" name="reliability" class="form-control" min="1" max="5" value="{{ $assessment->reliability ?? '' }}"></div>
            <div class="col-md-6"><label class="form-label">Bias Assessment</label><select name="bias" class="form-select">
