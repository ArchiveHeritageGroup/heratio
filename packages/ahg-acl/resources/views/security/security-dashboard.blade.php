{{-- Security Dashboard - Migrated from AtoM: ahgSecurityClearancePlugin/modules/securityClearance/templates/dashboardSuccess.php --}}
  ...
  <div class="card-body">
    <table class="table table-striped">
      ...
      <tbody>
        ...
        <tr class="{{ $exp->days_remaining <= 7 ? 'table-danger' : ($exp->days_remaining <= 14 ? 'table-warning' : '') }}">
          <td>{{ e($exp->username) }}</td>
          <td>
            <span class="badge" style="background-color: {{ $exp->color ?? '#666' }}">
              {{ e($exp->clearance_name) }}
            </span>
          </td>
          <td>{{ $exp->expires_at }}</td>
          <td>{{ $exp->days_remaining }}</td>
          <td>
            @if(($exp->renewal_status ?? '') === 'pending')
              <span class="badge bg-info">Pending</span>
            @elseif(($exp->renewal_status ?? '') === 'none')
...
<div class="card">
  ...
  <div class="card-body">
    <div class="row">
      <div class="col-md-3">
        <a href="{{ route('acl.security-index') }}" class="btn btn-outline-primary btn-block mb-2 w-100">
          <i class="fas fa-users"></i> Manage Clearances
        </a>
      </div>
      <div class="col-md-3">
        <a href="{{ route('acl.compartments') }}" class="btn btn-outline-secondary btn-block mb-2 w-100">
          <i class="fas fa-project-diagram"></i> Compartments
        </a>
      </div>
      <div class="col-md-3">
        <a href="{{ route('acl.security-audit-trail') }}" class="btn btn-outline-info btn-block mb-2 w-100">
          <i class="fas fa-history"></i> Audit Log
        </a>
      </div>
      <div class="col-md-3">
        <a href="{{ route('acl.security-report') }}" class="btn btn-outline-success btn-block mb-2 w-100">
