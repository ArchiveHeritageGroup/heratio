...
Two display-only fixes on the candidate card. The card body meta row no longer shows a dead muted string like `authority #901608`: when a candidate carries a `candidate_authority_id` it now renders the authority record's name (`candidate_display_name`), and when an `authority_slug` is present the name is wrapped in a clickable `<a href="/<slug>" target="_blank" rel="noopener">`. The slug is sourced from the existing `LEFT JOIN slug s ON s.object_id = c.candidate_authority_id` in `executeReview` (actors and taxonomy terms are both Qubit objects with `slug` rows; `slug.object_id` is unique so the join adds no row fan-out - all 1287 candidates with an authority id resolve to a slug). The no-slug fallback now shows the name as plain text and only emits the raw `#id` if there is genuinely no name. Second fix: `_evidenceRow.php` humanizes bare snake_case evidence tokens at display time - a value matching `^[a-z][a-z0-9_]*$` is rewritten with `ucfirst(str_replace('_', ' ', $token))`, so `candidate_has_no_meaningful_ancestors` renders as "Candidate has no meaningful ancestors". JSON blobs, mixed-case strings and already-readable text pass through untouched; stored evidence data and scoring are not changed. Files touched: `templates/_candidateCard.php` and `templates/_evidenceRow.php` only.
