        ...
        <div class="accordion-item">
          ...
          <div id="markdown-collapse" class="accordion-collapse collapse" aria-labelledby="markdown-heading">
            <div class="accordion-body">
              <div class="mb-3">
                <label class="form-label">Enable Markdown support <span class="badge bg-secondary ms-1">{{ __('Optional') }}</span></label>
                <div>
                  <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="settings[enabled]" id="md_no" value="0" {{ $settings['enabled'] != '1' ? 'checked' : '' }}>
                    <label class="form-check-label" for="md_no">No <span class="badge bg-secondary ms-1">{{ __('Optional') }}</span></label>
                  </div>
                  <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="settings[enabled]" id="md_yes" value="1" {{ $settings['enabled'] == '1' ? 'checked' : '' }}>
                    <label class="form-check-label" for="md_yes">Yes <span class="badge bg-secondary ms-1">{{ __('Optional') }}</span></label>
                  </div>
                </div>
              </div>
            </div>
