...
| **P5 — Sidecar + Mode B API** | Sidecar XML parser covering all four profiles, Mode A flat-sidecar layout, `scan_session_token` table, `/api/v2/scan/*` endpoints, wrapper scripts | 2.5 weeks | VueScan/NAPS2 post-scan hook creates fully-described IO via wrapper; all four sectors round-trip |
...
| **P7 — Hardening** | IIIF pyramid pre-generation, HTR integration, audio/video derivatives, 3D preview thumbnails, bulk-ops on inbox, audit-trail wiring, alternate standards ingress (EAD → archive profile, MARC → library profile, LIDO → gallery/museum profile, MODS → library profile) | 3 weeks | Throughput target: 10k pages/day; institutions using EAD/MARC/LIDO can drop native files into the watcher without transforming to heratioScan first |
