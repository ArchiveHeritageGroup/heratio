...
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #222;">
    ...
    <p>Your task <strong>{{ $context['step_name'] ?? '-' }}</strong> in the workflow <strong>{{ $context['workflow_name'] ?? '-' }}</strong> has been rejected by {{ $context['decision_by_name'] ?? 'a reviewer' }}. Please review the comment below and resubmit if appropriate.</p>
    ...
    <table style="width: 100%; border-collapse: collapse; background: #f8f9fa; padding: 15px; border-radius: 5px; margin: 15px 0;">
        ...
        @endif
        <tr><td style="padding: 6px 10px; color: #666; vertical-align: top;">Reviewer comment</td><td style="padding: 6px 10px;">{{ $context['comment'] ?? '-' }}</td></tr>
    </table>
