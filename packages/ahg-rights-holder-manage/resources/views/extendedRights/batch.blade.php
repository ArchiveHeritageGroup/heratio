...
<form method="post" action="{{ route('extended-rights.batch.store') }}" id="batch-rights-form">
        ...
        <div class="col-md-4">
          <div class="form-check">
            ...
            <label class="form-check-label" for="action_assign">
              <strong>{{ __('Assign Rights') }}</strong><br><small class="text-muted">{{ __('Apply rights to selected objects') }}</small>
        ...
        <div class="col-md-4">
          <div class="form-check">
            ...
            <label class="form-check-label" for="action_embargo">
              <strong>{{ __('Apply Embargo') }}</strong><br><small class="text-muted">{{ __('Restrict access to selected objects') }}</small>
             <span class="badge bg-secondary ms-1">{{ __('Optional') }}</span></label>
          </div>
        </div>
        <div class="col-md-4">
          <div class="form-check">
            ...
            <label class="form-check-label" for="action_clear">
              <strong>{{ __('Clear Rights') }}</strong><br><small class="text-muted">{{ __('Remove rights from selected objects') }}</small>
             <span class="badge bg-secondary ms-1">{{ __('Optional') }}</span></label>
          </div>
        </div>
      </div>
  ...
  <div class="card mb-4">
    ...
    <div class="card-body">
      <div class="mb-3">
        <label class="form-label">Search and select objects <span class="badge bg-secondary ms-1">{{ __('Optional') }}</span></label>
      ...
      <div class="form-check">
        ...
        <label class="form-check-label" for="overwrite">Overwrite existing rights <span class="badge bg-secondary ms-1">{{ __('Optional') }}</span></label>
      </div>
    </div>
  </div>
  ...
  <div id="assign_options" class="card mb-4">
    ...
    <div class="card-body">
      <div class="row">
        ...
        <div class="col-md-6 mb-3">
          <label for="creative_commons_id" class="form-label">Creative Commons License <span class="badge bg-secondary ms-1">{{ __('Optional') }}</span></label>
      ...
      {{-- TK Labels --}}
      <div class="mb-3">
        <label class="form-label">Traditional Knowledge Labels <span class="badge bg-secondary ms-1">{{ __('Optional') }}</span></label>
        <div class="row">
          @foreach($tkLabels ?? [] as $tk)
          <div class="col-md-4 mb-2">
            <div class="form-check">
              <input type="checkbox" name="tk_label_ids[]" value="{{ $tk->id }}" class="form-check-input" id="tk_{{ $tk->id }}">
              <label class="form-check-label" for="tk_{{ $tk->id }}">{{ $tk->name ?? $tk->code }} <span class="badge bg-secondary ms-1">{{ __('Required') }}</span></label>
            </div>
          </div>
          @endforeach
        </div>
  ...
  <section class="actions mb-3 nav gap-2" style="background-color:#495057;border-radius:.375rem;padding:1rem;">
    <a href="{{ route('extended-rights.dashboard') }}" class="btn atom-btn-outline-light"><i class="fas fa-arrow-left me-1"></i>{{ __('Cancel') }}</a>
    <button type="submit" class="btn atom-btn-outline-light"><i class="fas fa-check me-1"></i>{{ __('Execute Batch Operation') }}</button>
  </section>
</form>
