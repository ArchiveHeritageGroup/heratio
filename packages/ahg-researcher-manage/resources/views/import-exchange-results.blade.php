...
@section('content')
  <div class="multiline-header d-flex align-items-center mb-3"><i class="fas fa-3x fa-check-circle me-3" aria-hidden="true"></i><div class="d-flex flex-column"><h1 class="mb-0">{{ __('Import Results') }}</h1></div></div>
  @if(isset($rows) && count($rows))
    <div class="table-responsive"><table class="table table-bordered table-hover mb-0">
      <thead><tr><th>#</th><th>{{ __('Record') }}</th><th>{{ __('Status') }}</th><th>{{ __('Message') }}</th></tr></thead>
