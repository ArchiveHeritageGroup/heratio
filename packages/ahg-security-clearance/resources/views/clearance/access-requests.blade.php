...
{{-- Stats --}}
  <div class="row mb-4">
    <div class="col-md-3"><div class="card bg-warning text-dark"><div class="card-body"><h6>{{ __('Pending') }}</h6><h3>{{ $stats['pending'] ?? 0 }}</h3></div></div></div>
    <div class="col-md-3"><div class="card bg-success text-white"><div class="card-body"><h6>{{ __('Approved Today') }}</h6><h3>{{ $stats['approved_today'] ?? 0 }}</h3></div></div></div>
    <div class="col-md-3"><div class="card bg-danger text-white"><div class="card-body"><h6>{{ __('Denied Today') }}</h6><h3>{{ $stats['denied_today'] ?? 0 }}</h3></div></div></div>
    <div class="col-md-3"><div class="card bg-info text-white"><div class="card-body"><h6>{{ __('This Month') }}</h6><h3>{{ $stats['total_this_month'] ?? 0 }}</h3></div></div></div>
  </div>
  ...
  <div class="card">
    <div class="card-body table-responsive">
      <table class="table table-striped table-hover">
        <thead>
          <tr><th>{{ __('User') }}</th><th>{{ __('Object') }}</th><th>{{ __('Type') }}</th><th>{{ __('Priority') }}</th><th>{{ __('Justification') }}</th><th>{{ __('Requested') }}</th><th>{{ __('Actions') }}</th></tr>
        </thead>
        <tbody>
