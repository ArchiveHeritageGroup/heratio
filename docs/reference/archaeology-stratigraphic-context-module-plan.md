...
- **Phase 2 - stratigraphy. [BUILT - v1.154.446]** `archaeology_context_relationship` migration (both directions stored) + `ArchaeologyService::REL_TYPES` / `addRelationship` (reciprocity: above<->below, cuts<->cut_by, fills<->filled_by; same_as/bonds_with/abuts symmetric) / `removeRelationship` (drops the mirror) / `laterThanReaches` (cycle guard on the above/cuts/fills later-than graph) / `relationshipsForContext` / `contextPickList`. Relationship editor (add + remove, with the mirror kept automatically) on the context sheet. Verified: reciprocal auto-created, self-relations and stratigraphic loops refused.
- **Phase 3 - Harris Matrix. [BUILT - v1.154.447]** `ArchaeologyService::harrisMatrix` - union-find merge of `same_as` contexts, Kahn longest-path layering of the above/cuts/fills later-than edges into tiers (latest at top), cycle-safe (flags a same-as-induced loop rather than erroring). Rendered **server-side** as tiered boxes on the Stratigraphy page (`contexts` view) - deliberately NOT the CDN Mermaid path (blocked by CSP, cf. #1422/#1423); the Mermaid source is offered in a `<details>` for export. Verified: 1001 above 1002 above 1003 lays out as three tiers top-to-bottom.
- **Phase 4 - fill-out.**
  - **4a data-entry CRUD [BUILT - v1.154.447]** - the site and find forms the module lacked. `ArchaeologyService::saveSite`/`saveFind`/`sitePickList` + shared `createDescription` helper; `ArchaeologyController::siteCreate/siteEdit/siteSave/findCreate/findEdit/findSave/repositories`; `site-form`/`object-form` views. **Find form carries the context picker** (Phase 1's deferred item - finds link to `context_id`). Creating a site makes a top-level description; creating a find makes a description under its context (or site). "Add site"/"Add find"/"Edit" buttons wired into the sites/objects lists and the site/find detail views.
  - **4b remaining** - CSV import of contexts + relationships (Harris Matrix data commonly arrives from single-context recording spreadsheets); context-sheet PDF export; Elasticsearch + spatial indexing so contexts are searchable and mappable. Also: AJAX-refresh the find form's context list when the site changes (today it shows the preset site's contexts), and slot auto-created descriptions into the tree on save (currently next rebuild).
