...
<div class="row justify-content-center">
  <div class="col-md-6">
    <div class="card">
      <div class="card-header" style="background:var(--ahg-primary);color:#fff"><h4 class="mb-0"><i class="fas fa-upload me-2"></i>{{ __('Import AHG Settings') }}</h4></div>
      <div class="card-body">
        <div class="alert alert-info"><i class="fas fa-info-circle me-2"></i>{{ __('Upload a previously exported AHG settings JSON file to restore settings.') }}</div>
        ...
        <form method="post" enctype="multipart/form-data" action="{{ route('settings.ahg-import') }}">
          ...
          <div class="d-flex gap-2">
            <button type="submit" class="btn atom-btn-outline-success"><i class="fas fa-upload me-1"></i>{{ __('Import Settings') }}</button>
            <a href="{{ route('settings.index') }}" class="btn atom-btn-white"><i class="fas fa-times me-1"></i>{{ __('Cancel') }}</a>
          </div>
        </form>
      </div>
    </div>
