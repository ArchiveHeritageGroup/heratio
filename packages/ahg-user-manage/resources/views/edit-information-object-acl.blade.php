...
@section('title', __('User %1% - Edit information object permissions', ['%1%' => $user->username ?? '']))
...
@section('content')
  ...
  <form action="{{ route('user.editInformationObjectAcl', ['slug' => $user->slug]) }}" method="POST" id="editForm">
    @csrf

    <div class="section border rounded mb-3">
      <div class="section-heading rounded-top bg-light p-3">
        <h4 class="mb-0">{{ __('Edit information object permissions') }}</h4>
      </div>
      <div class="p-3">
          ...
          <table class="table table-bordered mb-3">
            ...
            <tbody>
              @foreach($permissions as $permission)
                <tr>
                  <td>{{ $permission->object_name ?? __('All information objects') }}</td>
                  <td>{{ $permission->repository_name ?? '-' }}</td>
        ...
        @else
          <p class="text-muted">{{ __('No information object permissions defined for this user.') }}</p>
        @endif

        <div class="mb-3">
          <label class="form-label">{{ __('Add information object permission') }}</label>
    ...
    <ul class="actions mb-3 nav gap-2">
      <li><a href="{{ route('user.indexInformationObjectAcl', ['slug' => $user->slug]) }}" class="btn atom-btn-outline-light" role="button">{{ __('Cancel') }}</a></li>
