...
/**
 * StoreLibraryOrderRequest - validate order create / update
 *
...
class StoreLibraryOrderRequest extends FormRequest
{
    ...
    {
        return [
            'lines.*.title.required_with' => 'Line title is required when adding order lines.',
            'lines.*.quantity.min'         => 'Line quantity must be at least 1.',
            'lines.*.unit_price.max'       => 'Line unit price exceeds maximum allowed.',
        ];
    }
}