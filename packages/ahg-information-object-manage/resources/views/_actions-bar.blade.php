...
@php
  ...
  // #1425: "Add new" creates a child in THIS record's own description standard
  // (its taxonomy-70 code), so children inherit the parent's standard. Null /
  // isad -> the plain ISAD create path.
  $_parentStandard = ($io->display_standard_id ?? null)
      ? \Illuminate\Support\Facades\DB::table('term')->where('id', $io->display_standard_id)->value('code')
      : null;
  $_addChildParams = ['parent_id' => $io->id]
      + ($_parentStandard && $_parentStandard !== 'isad' ? ['standard' => $_parentStandard] : []);
@endphp
<ul class="actions mb-3 nav gap-2">
  ...
  @endif
  @if($_canCreate)
  {{-- #1425: "Add new" creates a child in this record's own standard (children
       inherit the parent's description standard via ?standard=<code>&parent=). --}}
  <li>
    <a href="{{ route('informationobject.create', $_addChildParams) }}" class="btn atom-btn-outline-light">{{ __('Add new') }}</a>
  </li>
  <li>
