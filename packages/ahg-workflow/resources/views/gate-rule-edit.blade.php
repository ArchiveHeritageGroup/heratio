...
@section('content')
  ...
  <div class="card"><div class="card-header fw-semibold"><i class="fas fa-edit me-2"></i>{{ __('Edit Gate Rule') }}</div>
  <div class="card-body"><form method="POST" action="{{ $formAction ?? '#' }}">@csrf
    <div class="mb-3"><label class="form-label">Rule Name <span class="badge bg-secondary ms-1">{{ __('Required') }}</span></label><input type="text" class="form-control" name="name" required></div><div class="mb-3"><label class="form-label">Condition <span class="badge bg-secondary ms-1">{{ __('Recommended') }}</span></label><textarea class="form-control" name="condition" rows="2"></textarea></div><div class="mb-3"><label class="form-label">Priority <span class="badge bg-secondary ms-1">{{ __('Optional') }}</span></label><input type="number" class="form-control" name="priority"></div>
