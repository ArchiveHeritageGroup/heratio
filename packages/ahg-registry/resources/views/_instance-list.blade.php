{{--
  Software instance list (status dot, URL, type, sync, capacity).
  Vars: $instances (iterable), $canEdit (bool).
        ...
        @foreach ($instances as $inst)
            @php
                ...
                if (! empty($inst->last_heartbeat_at)) {
                    $hbDiff = time() - strtotime($inst->last_heartbeat_at);
                    $hbAgo = match (true) {
                        $hbDiff < 60     => __('just now'),
                        $hbDiff < 3600   => __(':n min ago', ['n' => (int) floor($hbDiff / 60)]),
                        $hbDiff < 86400  => __(':n hours ago', ['n' => (int) floor($hbDiff / 3600)]),
                        default          => __(':n days ago', ['n' => (int) floor($hbDiff / 86400)]),
                    };
                }
                $editHref = $editable && \Illuminate\Support\Facades\Route::has('registry.myInstitutionInstanceEdit')
                ...
                <div class="d-flex align-items-start">
                    ...
                    <div class="flex-grow-1 min-width-0">
                        ...
                        @if (! empty($inst->sync_enabled))
                            <div class="mt-1">
                                <small class="text-muted">
                                    ...
                                    @if ($hbAgo){{ __('Last sync: :ago', ['ago' => $hbAgo]) }}@else{{ __('Never synced') }}@endif
                                </small>
                            </div>
                        @endif
                        ...
                        @if (! empty($inst->description))
                            <div class="mt-1">
                                <small class="text-muted">{{ \Illuminate\Support\Str::limit($inst->description, 200) }}</small>
                            </div>
                        @endif
                    ...
                    <div class="ms-2 flex-shrink-0">
                        ...
                        @if ($editHref)
                            <a href="{{ $editHref }}" class="btn btn-sm btn-outline-secondary" title="{{ __('Edit') }}"><i class="fas fa-edit"></i></a>
                        @endif
                    </div>
                </div>
            </div>
