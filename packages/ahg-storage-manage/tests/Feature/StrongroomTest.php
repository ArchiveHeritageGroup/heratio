...
/**
 * Wraps every test in a transaction (DatabaseTransactions) so the heratio
 * DB is never polluted by leftover test rows.
 */
class StrongroomTest extends TestCase
{
    ...
    public function test_unique_slug_collision_appends_suffix(): void
    {
        ...
        $s1 = $this->service->getById($id1)->slug;
        $s2 = $this->service->getById($id2)->slug;
        $s3 = $this->service->getById($id3)->slug;

        $this->assertSame('same-name', $s1);
        $this->assertSame('same-name-2', $s2);
        $this->assertSame('same-name-3', $s3);
    }
    ...
    public function test_create_normalises_invalid_capacity_unit(): void
    {
        $id = $this->service->create([
            'name'          => 'Unit Coercion',
            'capacity_unit' => 'parsecs',  // not in the allowed set
        ]);
    ...
    public function test_create_requires_name(): void
    {
        ...
        $this->service->create(['name' => '   ']);  // whitespace-only
    }
    ...
    public function test_update_preserves_slug_and_normalises_unit(): void
    {
        ...
        $this->service->update($id, [
            'name'          => 'Renamed',
            'capacity_unit' => 'parsecs',  // invalid -> coerced
            'capacity_value' => 50,
        ]);
    ...
    public function test_capacity_overflow_excludes_re_assignment_double_count(): void
    {
        ...
        // Re-assigning the SAME physical object to size 9 should not double-count
        // the existing 8 — projected total is 9, within capacity.
    ...
    {
        $roomId = $this->service->create(['name' => 'No Cap']);  // no capacity_value
    ...
    public function test_delete_with_occupants_is_blocked(): void
    {
        ...
        $this->assertNotNull($this->service->getById($roomId), 'Room must still exist after blocked delete');
    }
    ...
    public function test_browse_returns_room_with_utilisation(): void
    {
        ...
        $this->assertNotEmpty($rows, 'Browse must surface the test room');
    ...
    /**
     * Create a minimal physical_object row for FK satisfaction. AtoM models
     * physical_object as a subtype of `object` (single-table inheritance via
     * a shared id), and `object.id` is the AUTO_INCREMENT root. So the two-row
     * insert pattern: insert into `object` (auto id), then insert into
