...
@section('content')
  <div class="card border-danger"><div class="card-header bg-danger text-white"><i class="fas fa-exclamation-triangle me-2"></i>{{ __('Confirm Delete') }}</div><div class="card-body">
    ...
    <form method="POST" action="{{ $deleteUrl ?? '#' }}">@csrf @method('DELETE')
      <div class="d-flex gap-2"><button type="submit" class="btn atom-btn-outline-danger"><i class="fas fa-trash me-1"></i> {{ __('Delete') }}</button><a href="{{ url()->previous() }}" class="btn atom-btn-outline-light"><i class="fas fa-times me-1"></i> {{ __('Cancel') }}</a></div>
    </form>
  </div></div>
@endsection
