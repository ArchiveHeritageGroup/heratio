...
@section('content')
  ...
  <div class="card"><div class="card-header fw-semibold" style="background:var(--ahg-primary);color:#fff"><i class="fas fa-edit me-2"></i>{{ __('Restore from Upload') }}</div>
  <div class="card-body"><form method="POST" action="{{ $formAction ?? '#' }}">@csrf
    <div class="mb-3"><label class="form-label">Backup File <span class="badge bg-secondary ms-1">{{ __('Required') }}</span></label><input type="file" class="form-control" name="backup_file" accept=".zip,.sql,.gz" required></div><div class="alert alert-warning"><i class="fas fa-exclamation-triangle me-2"></i>{{ __('This will overwrite existing data.') }}</div>
