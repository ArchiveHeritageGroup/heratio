...
- No import/export for the museum-procedure condition data or related interchange formats (CSV/JSON mapping or IIIF annotations for photographed damage) and no mapping into RiC for activity/event relations.
...
- packages/ahg-ric (exporter)
  - Evidence: RiC exporter supports Activity/Agent/Event but has no mapping for Museum Condition entities. Add: exporter mapping for Condition -> Event/RecordResource paths and include photos as evidence nodes.
...
- Add an import/export routine: CSV ↔ museum-procedure mapping and a small JSON-LD mapping into RiC/OA-like nodes for interoperability.
