...
@section('title', __('User %1% - Actor permissions', ['%1%' => $user->username ?? '']))
...
@section('content')
  ...
  <div class="section">
          ...
          @foreach($acl as $objectId => $actions)
            <tr>
              <td colspan="{{ $tableCols }}"><strong>
                @if($objectId > 1 && isset($actorNames[$objectId]))
                  {{ $actorNames[$objectId] }}
                @else
                  <em>{{ __('All %1%', ['%1%' => lcfirst(config('app.ui_label_actor', 'actors'))]) }}</em>
                @endif
              </strong></td>
            </tr>
            @php $row = 0; @endphp
    ...
    @else
      <p class="text-muted p-3">{{ __('No actor permissions defined for this user.') }}</p>
    @endif
  </div>
