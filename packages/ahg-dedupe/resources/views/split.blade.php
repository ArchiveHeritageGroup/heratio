...
@section('content')
  ...
  <div class="card-body"><form method="POST" action="{{ $formAction ?? '#' }}">@csrf
    <div class="mb-3"><label class="form-label">Records to Split <span class="badge bg-secondary ms-1">Required</span></label><textarea class="form-control" name="split_names" rows="4" placeholder="{{ __('One name per line') }}"></textarea></div>
