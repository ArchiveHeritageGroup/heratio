...
CREATE TABLE IF NOT EXISTS `ahg_vendors` (
    ...
    -- #1264: PII columns are widened to VARCHAR(512) so they can hold the
    -- ENC2: field-level ciphertext (~205-233 chars) when encryption is on.
    -- Plaintext values are unaffected.
    `phone` VARCHAR(512) DEFAULT NULL,
    `phone_alt` VARCHAR(512) DEFAULT NULL,
    `fax` VARCHAR(512) DEFAULT NULL,
    `email` VARCHAR(512) DEFAULT NULL,
    `website` VARCHAR(255) DEFAULT NULL,
    `bank_name` VARCHAR(512) DEFAULT NULL,
    `bank_branch` VARCHAR(512) DEFAULT NULL,
    `bank_account_number` VARCHAR(512) DEFAULT NULL,
    `bank_branch_code` VARCHAR(512) DEFAULT NULL,
    `bank_account_type` VARCHAR(512) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `ahg_vendor_contacts` (
    ...
    -- #1264: widened for ENC2: field-level ciphertext (see ahg_vendors above).
    `phone` VARCHAR(512) DEFAULT NULL,
