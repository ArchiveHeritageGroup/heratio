...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#newJobModal"><i class="fas fa-plus me-1"></i>New Job</button>
</div>
@if(!empty($jobs))
<div class="card">
...
<div class="modal fade" id="newJobModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><form method="POST">@csrf
    ...
    <div class="modal-body">
        ...
        <div class="mb-3"><label class="form-label">Extraction Type</label><select name="extraction_type" class="form-select"><option value="ocr">OCR</option><option value="ner">Named Entity Recognition</option><option value="metadata">Metadata Extraction</option><option value="full_text">Full Text</option></select></div>
        <div class="mb-3"><label class="form-label">Source Collection</label><select name="collection_id" class="form-select"><option value="">-- Select --</option>@foreach($collections ?? [] as $c)<option value="{{ $c->id }}">{{ e($c->name ?? '') }}</option>@endforeach</select></div>
    </div>
    <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button><button type="submit" class="btn btn-primary">Start Job</button></div>
</form></div></div></div>
@endsection