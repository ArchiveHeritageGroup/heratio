...
<form method="POST" action="{{ route('security-clearance.watermark-settings-store') }}">
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="mb-3 form-check">
              ...
              <label class="form-check-label" for="defaultEnabled">{{ __('Enable watermarks by default') }}</label>
            </div>
            <div class="mb-3">
              <label class="form-label">{{ __('Default Watermark Type') }}</label>
              <select name="default_type" class="form-select">
                <option value="COPYRIGHT" {{ ($settings['default_type'] ?? '') === 'COPYRIGHT' ? 'selected' : '' }}>{{ __('Copyright') }}</option>
                <option value="CONFIDENTIAL" {{ ($settings['default_type'] ?? '') === 'CONFIDENTIAL' ? 'selected' : '' }}>{{ __('Confidential') }}</option>
                <option value="DRAFT" {{ ($settings['default_type'] ?? '') === 'DRAFT' ? 'selected' : '' }}>{{ __('Draft') }}</option>
                <option value="CUSTOM" {{ ($settings['default_type'] ?? '') === 'CUSTOM' ? 'selected' : '' }}>{{ __('Custom') }}</option>
              </select>
            </div>
            <div class="mb-3">
          ...
          <div class="col-md-6">
            <div class="mb-3 form-check">
              ...
              <label class="form-check-label" for="applyOnView">{{ __('Apply watermark on view') }}</label>
            </div>
            <div class="mb-3 form-check">
              ...
              <label class="form-check-label" for="applyOnDownload">{{ __('Apply watermark on download') }}</label>
            </div>
            <div class="mb-3 form-check">
    ...
    <div class="card mb-4">
      <div class="card-header"><h5 class="mb-0">{{ __('Available Watermark Types') }}</h5></div>
      <div class="card-body table-responsive">
        <table class="table table-sm">
          <thead><tr><th>{{ __('Name') }}</th><th>{{ __('Code') }}</th><th>{{ __('Preview') }}</th><th>{{ __('Status') }}</th></tr></thead>
          <tbody>
    ...
    <div class="card mb-4">
      <div class="card-header"><h5 class="mb-0">{{ __('Custom Watermarks') }}</h5></div>
      <div class="card-body table-responsive">
        <table class="table table-sm">
          <thead><tr><th>{{ __('Name') }}</th><th>{{ __('File') }}</th><th>{{ __('Position') }}</th><th>{{ __('Opacity') }}</th></tr></thead>
          <tbody>
