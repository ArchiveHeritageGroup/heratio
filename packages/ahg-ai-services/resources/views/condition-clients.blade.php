...
@section('content')
  <div class="multiline-header d-flex align-items-center mb-3">
    ...
    <div class="d-flex flex-column"><h1 class="mb-0">{{ __('Condition Clients') }}</h1></div>
  </div>

  @if(isset($rows) && count($rows))
    <div class="table-responsive"><table class="table table-bordered table-hover mb-0">
      <thead><tr><th>#</th><th>{{ __('Details') }}</th><th>{{ __('Status') }}</th><th>{{ __('Date') }}</th><th>{{ __('Actions') }}</th></tr></thead>
