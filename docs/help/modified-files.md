...
# Modified Heratio Core Files

This document describes the files that differ from a pure Heratio 2.10 installation when using the AHG Framework.
...
The install script replaces Heratio's default `ProjectConfiguration.class.php` with a custom version that includes:
...
- **Before:** Heratio loads plugins from `setting_i18n` table (id=1), a serialized PHP array
...
- Backwards compatibility with Heratio core code
...
**Problem:** Heratio's default behavior removes enabled plugins from the themes list. When `ahgThemeB5Plugin` is enabled, it disappears from Admin → Themes, making it impossible to switch or view theme settings.
...
### Standard Heratio Configuration
```nginx
...
```ini
; Heratio Framework - auto-prepend (optional, not currently used)
...
If patches need to be reapplied (e.g., after Heratio upgrade):
```bash
...
## After Heratio Upgrades

When upgrading Heratio core:
