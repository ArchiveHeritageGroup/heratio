...
class BlogLinkService
{
    ...
    /** Articles linked to $postId in EITHER direction (bidirectional). */
    public static function related(int $postId): array
    {
        if (! Schema::hasTable('blog_post_link')) {
            return [];
        }
        $ids = DB::table('blog_post_link')->where('post_id', $postId)->pluck('related_post_id')
            ->merge(DB::table('blog_post_link')->where('related_post_id', $postId)->pluck('post_id'))
            ->map(fn ($v) => (int) $v)->unique()->values();
        if ($ids->isEmpty()) {
            return [];
        }

        return DB::table('blog_post')->whereIn('id', $ids)
            ->select('id', 'slug', 'title', 'status')->orderBy('title')
            ->get()->map(fn ($r) => (array) $r)->all();
    }

    public static function add(int $postId, int $targetId): bool
    {
