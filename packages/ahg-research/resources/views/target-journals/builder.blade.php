...
<div class="container py-3" style="max-width: 860px;">
  ...
  <form method="post" action="{{ $journal ? route('research.target-journal.update', $journal['id']) : route('research.target-journal.store') }}">
    ...
    <div class="row">
      ...
      <div class="col-md-3 mb-3"><label class="form-label">{{ __('ISSN') }}</label>
    ...
    <div class="row">
      <div class="col-md-6 mb-3"><label class="form-label">{{ __('Article types') }}</label>
        <input name="article_types" class="form-control" value="{{ old('article_types', $journal['article_types'] ?? '') }}" placeholder="{{ __('research, review, case study') }}"></div>
    ...
    <div class="row">
      <div class="col-md-8 mb-3"><label class="form-label">{{ __('Accreditation / indexing') }}</label>
        <input name="accreditation" class="form-control" value="{{ old('accreditation', $journal['accreditation'] ?? '') }}" placeholder="{{ __('DHET, Scopus, Web of Science, DOAJ, Sabinet') }}"></div>
      <div class="col-md-4 mb-3"><label class="form-label">{{ __('Accreditation market') }}</label>
        <input name="accreditation_market" class="form-control" value="{{ old('accreditation_market', $journal['accreditation_market'] ?? '') }}" placeholder="{{ __('ZA') }}"></div>
    </div>
    ...
    <div class="row">
      <div class="col-md-4 mb-3"><label class="form-label">{{ __('Peer review') }}</label>
        <input name="peer_review" class="form-control" value="{{ old('peer_review', $journal['peer_review'] ?? '') }}" placeholder="{{ __('double-blind') }}"></div>
      ...
      <div class="col-md-3 mb-3"><label class="form-label">{{ __('APC') }}</label>
