...
└──────────────────────────────────────────────────────────────────────────────────────────────────────┘
  ...
  ══════════════════════════════════════════════════════════════════════
   ...
   GLAM/DAM & INFORMATION OBJECT LINKS:
     audit_log.object_id ──────────────► object.id (Heratio core — any entity)
     ahg_audit_log.entity_type ────────► informationobject | actor | accession | repository | ...
     ahg_audit_log.entity_id ──────────► Polymorphic FK to any Heratio entity
     ...
     All tables: user_id ──────────────► user.id (extends actor.id in Heratio)
  ══════════════════════════════════════════════════════════════════════
```
...
└──────────────────────────────────────────────────────────────────────────────────────────────────────────┘
  ...
  ════════════════════════════════════════════════════════════════════════════════════════
   GLAM/DAM & INFORMATION OBJECT LINKS:
     ...
     privacy_officer.user_id ────────────► user.id (extends actor.id in Heratio)
...
└─────────────────────────────────────────────────────────────────────────────────────────────────────────┘
  ...
  │     user_security_clearance.user_id ─────────► user.id (extends actor.id in Heratio)         │
...
└──────────────────────────────────────────────────────────────────────────────────────────────────────┘
  ...
  ════════════════════════════════════════════════════════════════════════
   GLAM/DAM & INFORMATION OBJECT LINKS:
     ...
     condition_report.assessor_user_id ─────► user.id (extends actor.id in Heratio)
...
└──────────────────────────────────────────────────────────────────────────────────────────────────────┘
  ...
  │    repository_id ───────► repository.id (Heratio core — archival institution)       │
  ...
  │                                                                                  │
  │  ahg_loan_object                                                                 │
  │    information_object_id ► information_object.id (Heratio core — archival record)   │
  │    external_object_id ──► External ref for non-Heratio objects (gallery/museum)     │
...
```
  ...
  ════════════════════════════════════════════════════════════════════════════════════════
   ...
   GLAM/DAM & INFORMATION OBJECT LINKS:
     ...
     heritage_asset.object_id ──────────────► object.id (Heratio core entity)
     ...
     heritage_audit_log.user_id ────────────► user.id (extends actor.id in Heratio)
...
```
  ...
  ════════════════════════════════════════════════════════════════════════════════════════
   ...
   GLAM/DAM & INFORMATION OBJECT LINKS:
     ...
     iiif_auth_token.user_id ──────────► user.id (extends actor.id in Heratio)
...
```
  ...
  ════════════════════════════════════════════════════════════════════════════════════════
   ...
   GLAM/DAM & INFORMATION OBJECT LINKS:
     ...
     research_researcher.user_id ──────────► user.id (extends actor.id in Heratio)
...
```
  ...
  ════════════════════════════════════════════════════════════════════════════════════════
   ...
   GLAM/DAM & INFORMATION OBJECT LINKS:
     ...
     registry_favorite.user_id ────────► user.id (Heratio user)
...
└──────────────────────────────────────────┘
  ...
  GLAM/DAM & INFORMATION OBJECT LINKS:
   ...
   NOTE: RiC sync tracks ANY Heratio entity type for triplestore synchronization
  ════════════════════════════════════════════════════════════════════════════════════════
```
...
All plugin tables that reference `information_object.id` — the central archival record in Heratio.
...
Tables that link to `repository.id` — the archival institution in Heratio.
...
Tables that link to `actor.id` — persons, organizations, families in Heratio.
...
Tables that link to `user.id` (user extends actor in Heratio).
...
│                         ├── archive ──► information_object (default Heratio)                 │
...
```
audit_log.object_id ──────────► any Heratio object.id (field-level changes)
ahg_audit_log.entity_type ────► informationobject | actor | accession | repository | ...
