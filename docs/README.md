# Heratio Documentation

Documentation for **Heratio** - a standalone Laravel 12 GLAM (Gallery, Library, Archive, Museum) platform.

## Sections

- [Getting Started](getting-started/README.md)
- [Installation](install/README.md)
- [User Manual](user-manual/README.md)
- [Technical](technical/README.md)
- [Developer](developer/README.md)
- [Operations](operations/README.md)
- [Reference](reference/README.md)
- [ADRs](adr/README.md)

## Key User Guides

| Module | Guide |
|---|---|
| Research Portal | [researcher-user-guide.md](researcher-user-guide.md) |
| GLAM Browse | [glam-browse-user-guide.md](glam-browse-user-guide.md) |
| Condition Assessment | [condition-assessment-user-guide.md](condition-assessment-user-guide.md) |
| Provenance | [provenance-user-guide.md](provenance-user-guide.md) |
| Cart & E-Commerce | [cart-ecommerce-user-guide.md](cart-ecommerce-user-guide.md) |
| ODRL Rights | [rights-management-user-guide.md](rights-management-user-guide.md) |
| Heritage Accounting | [heritage-accounting-user-guide.md](heritage-accounting-user-guide.md) |
| Spectrum 5.1 | [spectrum-user-guide.md](spectrum-user-guide.md) |
| AI Tools | [ai-tools-user-guide.md](ai-tools-user-guide.md) |
| API | [api-user-guide.md](api-user-guide.md) |
| RiC | [ric-user-guide.md](ric-user-guide.md) |
| Help System | 221+ searchable articles at `/help` |

## Architecture

See [CLAUDE.md](../CLAUDE.md) for full architecture, database rules, and development standards.

## Fresh Install

See [install/README.md](install/README.md) or the Fresh Install Procedure section in [CLAUDE.md](../CLAUDE.md).
