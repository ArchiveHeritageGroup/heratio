...
/**
 * DcManageController - Controller for Heratio
 *
...
class DcManageController extends Controller
{
     ...
     *
     * DC fields: accessConditions, extentAndMedium, identifier, language,
    ...
    public function edit(Request $request, string $slug)
    {
        ...
        // DC Names (creators via events type 111)
        ...
        // DC Type terms (taxonomy 62)
        $dcTypes = DB::table('object_term_relation')
            ->join('term_i18n', 'object_term_relation.term_id', '=', 'term_i18n.id')
            ->join('term', 'object_term_relation.term_id', '=', 'term.id')
            ->where('object_term_relation.object_id', $io->id)
            ->where('term.taxonomy_id', 62)
            ->where('term_i18n.culture', $culture)
            ->select('term.id as term_id', 'term_i18n.name')
            ->get();
    ...
    private function getFormDropdowns(string $culture): array
    {
        ...
        // DC Type options (taxonomy 62)
        $dcTypeOptions = DB::table('term')
            ->join('term_i18n', 'term.id', '=', 'term_i18n.id')
            ->where('term.taxonomy_id', 62)
            ->where('term_i18n.culture', $culture)
            ->orderBy('term_i18n.name')
        ...
        return compact('repositories', 'displayStandards', 'eventTypes', 'dcTypeOptions');
    }
