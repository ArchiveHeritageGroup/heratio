...
<div class="card">
    <div class="card-body">
        <form method="POST">
            @csrf
            <div class="row mb-3">
                <div class="col-md-6"><label class="form-label">Name <span class="text-danger">*</span></label><input type="text" name="name" class="form-control" required value="{{ e($type->name ?? '') }}"></div>
                <div class="col-md-3"><label class="form-label">Code <span class="text-danger">*</span></label><input type="text" name="code" class="form-control" required value="{{ e($type->code ?? '') }}"></div>
                <div class="col-md-3"><label class="form-label">Sort Order</label><input type="number" name="sort_order" class="form-control" value="{{ $type->sort_order ?? 0 }}"></div>
            </div>
            <div class="mb-3"><label class="form-label">Description</label><textarea name="description" class="form-control" rows="2">{{ e($type->description ?? '') }}</textarea></div>
            <h5 class="border-bottom pb-2 mt-4 mb-3">Privileges</h5>
            <div class="row mb-3">
                <div class="col-md-4"><label class="form-label">Max Advance Booking Days</label><input type="number" name="max_advance_days" class="form-control" value="{{ $type->max_advance_days ?? 30 }}"></div>
                <div class="col-md-4"><label class="form-label">Max Hours per Day</label><input type="number" name="max_hours_per_day" class="form-control" value="{{ $type->max_hours_per_day ?? 8 }}"></div>
                <div class="col-md-4"><label class="form-label">Max Materials</label><input type="number" name="max_materials" class="form-control" value="{{ $type->max_materials ?? 5 }}"></div>
            </div>
