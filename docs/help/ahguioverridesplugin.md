...
The ahgUiOverridesPlugin provides a centralized location for UI action overrides and helper function customizations in Heratio. This plugin intercepts and extends core Heratio module actions without modifying base Heratio files, following the AHG Framework architecture principles.
...
```json
{
    ...
    "version": "1.0.0",
    "description": "UI action overrides for Heratio modules - centralized location for action customizations",
...
{
    public static $summary = 'UI action overrides for Heratio modules';
