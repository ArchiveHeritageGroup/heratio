        ...
        <div class="tab-content">
          ...
          <div class="tab-pane fade {{ ($selectedType ?? '')===$opp['code']?'show active':'' }}" id="form-{{ $opp['code'] }}" role="tabpanel">
            ...
            <form class="contribution-form">@csrf
              ...
              <div class="mb-3"><label class="form-label">Transcription <span class="text-danger">*</span> <span class="badge bg-danger ms-1">Required</span></label><textarea class="form-control font-monospace" name="content[text]" rows="12" required minlength="10" placeholder="{{ __('Type the text exactly as it appears...') }}"></textarea><div class="form-text">Use [...] for unclear words and [illegible] for unreadable sections.</div></div>
              @elseif($opp['code']==='identification')
              <div class="mb-3"><label class="form-label">Name <span class="text-danger">*</span> <span class="badge bg-danger ms-1">Required</span></label><input type="text" class="form-control" name="content[name]" required minlength="2" placeholder="{{ __('Full name of the person identified') }}"></div>
              <div class="row mb-3"><div class="col-md-6"><label class="form-label">Relationship to Image <span class="badge bg-secondary ms-1">Optional</span></label><select class="form-select" name="content[relationship]"><option value="">{{ __('Select...') }}</option><option value="subject">{{ __('Subject (pictured)') }}</option><option value="photographer">{{ __('Photographer') }}</option><option value="owner">{{ __('Owner/Donor') }}</option></select></div><div class="col-md-6"><label class="form-label">Position in Image <span class="badge bg-secondary ms-1">Optional</span></label><input type="text" class="form-control" name="content[position]" placeholder="{{ __('e.g., Front row, left') }}"></div></div>
              <div class="mb-3"><label class="form-label">How confident are you? <span class="badge bg-secondary ms-1">Optional</span></label><select class="form-select" name="content[confidence]"><option value="certain">{{ __('Certain') }}</option><option value="likely">{{ __('Likely') }}</option><option value="possible">{{ __('Possible') }}</option></select></div>
              @elseif($opp['code']==='context')
              <div class="mb-3"><label class="form-label">Type of Context <span class="badge bg-secondary ms-1">Optional</span></label><select class="form-select" name="content[context_type]"><option value="historical">{{ __('Historical Background') }}</option><option value="personal">{{ __('Personal Memory') }}</option><option value="location">{{ __('Location Information') }}</option><option value="event">{{ __('Event Details') }}</option></select></div>
              <div class="mb-3"><label class="form-label">Your Context <span class="text-danger">*</span> <span class="badge bg-danger ms-1">Required</span></label><textarea class="form-control" name="content[text]" rows="8" required minlength="20" placeholder="{{ __('Share what you know...') }}"></textarea></div>
              @elseif($opp['code']==='correction')
              <div class="mb-3"><label class="form-label">Field to Correct <span class="text-danger">*</span> <span class="badge bg-danger ms-1">Required</span></label><select class="form-select" name="content[field]" required><option value="">{{ __('Select field...') }}</option><option value="title">{{ __('Title') }}</option><option value="date">{{ __('Date') }}</option><option value="description">{{ __('Description') }}</option><option value="names">{{ __('Names') }}</option><option value="location">{{ __('Location') }}</option></select></div>
              <div class="mb-3"><label class="form-label">Suggested Correction <span class="text-danger">*</span> <span class="badge bg-danger ms-1">Required</span></label><textarea class="form-control" name="content[suggestion]" rows="3" required></textarea></div>
              <div class="mb-3"><label class="form-label">Reason <span class="text-danger">*</span> <span class="badge bg-danger ms-1">Required</span></label><textarea class="form-control" name="content[reason]" rows="2" required></textarea></div>
              @elseif($opp['code']==='translation')
              <div class="mb-3"><label class="form-label">Target Language <span class="text-danger">*</span> <span class="badge bg-danger ms-1">Required</span></label><select class="form-select" name="content[target_language]" required><option value="">{{ __('Select language...') }}</option>@foreach(['af'=>'Afrikaans','zu'=>'Zulu','xh'=>'Xhosa','st'=>'Sesotho','tn'=>'Setswana','en'=>'English'] as $code=>$name)<option value="{{ $code }}">{{ $name }}</option>@endforeach</select></div>
              <div class="mb-3"><label class="form-label">Translation <span class="text-danger">*</span> <span class="badge bg-danger ms-1">Required</span></label><textarea class="form-control" name="content[text]" rows="8" required placeholder="{{ __('Your translation...') }}"></textarea></div>
              @endif
