  ...
  <div class="col-md-6 col-lg-5">
    <div class="card border-0 shadow-sm">
      <div class="card-body text-center py-5">
        ...
        <h2 class="h4 mt-4">{{ __('Verification Failed') }}</h2>
        <p class="text-muted mb-4">{{ $error ?? 'The verification link is invalid or has expired.' }}</p>
        <a href="{{ route('heritage.contributor-register') }}" class="btn atom-btn-secondary"><i class="fas fa-user-plus me-2"></i>{{ __('Register Again') }}</a>
        @endif
      </div>
    </div>
  </div>
