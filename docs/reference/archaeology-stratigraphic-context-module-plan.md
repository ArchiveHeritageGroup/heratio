...
- **Phase 1 - the layer entity.** `archaeology_context` migration + `context_type`/`phase` taxonomy terms + context CRUD + context sheet view + drawings via the existing uploader. Add `context_id` to `archaeology_object`, backfill, context picker on the find form. **This alone delivers structured layers with drawings and finds.**
