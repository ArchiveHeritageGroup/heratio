    ...
    <nav class="card-body border-bottom p-2 small" aria-label="{{ __('Pagination') }}">
      ...
      <ul class="pagination pagination-sm justify-content-center mb-2">
        <li class="page-item {{ $actorPager->currentPage() <= 1 ? 'disabled' : '' }}">
          <a class="page-link page-link-prev" href="{{ $actorPager->currentPage() > 1 ? request()->fullUrlWithQuery(['actors_page' => $actorPager->currentPage() - 1]) : '#' }}" aria-label="{{ __('Previous') }}">
        ...
        <li class="page-item {{ $actorPager->currentPage() >= $actorPager->lastPage() ? 'disabled' : '' }}">
          <a class="page-link page-link-next" href="{{ $actorPager->currentPage() < $actorPager->lastPage() ? request()->fullUrlWithQuery(['actors_page' => $actorPager->currentPage() + 1]) : '#' }}" aria-label="{{ __('Next') }}">
