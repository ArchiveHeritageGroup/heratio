...
(function(){
  ...
  document.getElementById('nerExtractBtn').addEventListener('click',function(){
    ...
    .then(function(d){
      ...
      var h='';for(var tp in ent){var ic=icons[tp]||'fa-tag',cl=colors[tp]||'secondary';h+='<div class="mb-2"><h6><i class="fas '+ic+' me-1 text-'+cl+'"></i>'+labelFor(tp)+' <span class="badge bg-'+cl+'">'+ent[tp].length+'</span></h6><div class="d-flex flex-wrap gap-1">';ent[tp].forEach(function(e){h+='<span class="badge bg-'+cl+' bg-opacity-10 text-'+cl+' border border-'+cl+'">'+e+'</span>';});h+='</div></div>';}
