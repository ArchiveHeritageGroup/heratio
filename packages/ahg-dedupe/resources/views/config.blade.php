...
@section('content')
  ...
  <div class="card-body"><form method="POST" action="{{ $formAction ?? '#' }}">@csrf
    <div class="mb-3"><label class="form-label">Auto-Scan Interval (days) <span class="badge bg-secondary ms-1">Optional</span></label><input type="number" class="form-control" name="auto_scan_interval"></div><div class="mb-3"><label class="form-label">Min Confidence (%) <span class="badge bg-secondary ms-1">Optional</span></label><input type="number" class="form-control" name="min_confidence"></div>
