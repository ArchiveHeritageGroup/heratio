...
**Framework:** Heratio AHG Framework (Laravel Query Builder + Symfony 1.x)
...
6. **Import** → Records created in Heratio database
...
The plugin includes sector-specific CSV exporters for both transformation (during import) and batch export of existing Heratio records.
...
The `exportFromDatabase()` method allows exporting directly from Heratio database:
```php
abstract class BaseExporter
{
    // ... existing methods
...
The `batchExportAction` allows exporting existing Heratio records:
...
Exports Heratio records to Preservica formats.
```php
...
#### Method 1: Native Heratio (Default) - `generate_derivatives: true`
...
Files are copied to Heratio's standard structure:
```
...
Heratio generates derivatives for:
