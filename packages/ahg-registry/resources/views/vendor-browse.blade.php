...
@section('title', 'Vendors Directory')
...
<nav aria-label="breadcrumb" class="mb-3">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active">{{ __('Vendors Directory') }}</li>
  </ol>
</nav>

<div class="row mb-4 align-items-center">
  <div class="col">
    <h1 class="h3 mb-1"><i class="fas fa-handshake me-2"></i>{{ __('Vendors Directory') }}</h1>
    <p class="text-muted mb-0">{{ __(':count items', ['count' => number_format($result['total'] ?? 0)]) }}</p>
  </div>
  @auth
  <div class="col-auto">
    <a href="{{ route('registry.vendorRegister') }}" class="btn atom-btn-white btn-sm"><i class="fas fa-plus me-1"></i>{{ __('Add') }}</a>
  </div>
  @endauth
</div>
