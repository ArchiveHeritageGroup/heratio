...
- **Sync API** -- Automated registration and heartbeat monitoring for remote Heratio/Heratio instances
...
| **System Administrators** | Register Heratio/Heratio instances, enable sync monitoring, track software versions |
...
- **Technical details** -- Descriptive standards used, management system, whether they use Heratio
...
- **Instances** -- Registered Heratio/Heratio instances with status indicators
...
| Uses Heratio | No | Whether the institution uses Heratio |
...
- **Instances** -- Registered Heratio/Heratio instances
...
| Institution URL | Main website (distinct from Heratio instance URL) |
...
Instances are the actual Heratio or Heratio installations your institution runs.
...
| Feature usage | No | Checkboxes for which Heratio features are in use (accessions, authority records, physical storage, etc.) |
...
The Sync API enables remote Heratio/Heratio instances to register with the Registry and send periodic health updates. All API endpoints use JSON request/response bodies.
...
Each instance can report which Heratio features it uses through the `registry_instance_feature` table. This enables:
...
3. **Register your instances** -- Document your Heratio/Heratio installations. This helps the community understand deployment patterns and helps vendors identify potential clients.
...
| `registry_instance` | Heratio/Heratio instances (URL, version, hosting, sync) |
