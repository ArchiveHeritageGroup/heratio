...
@section('content')
  <div class="multiline-header d-flex align-items-center mb-3">
    ...
    <div class="d-flex flex-column"><h1 class="mb-0">{{ __('Authority Configuration') }}</h1></div>
  </div>
  <div class="card"><div class="card-header fw-semibold" style="background:var(--ahg-primary);color:#fff"><i class="fas fa-edit me-2"></i>{{ __('Authority Configuration') }}</div>
  <div class="card-body"><form method="POST" action="{{ $formAction ?? '#' }}">@csrf
    <div class="mb-3"><label class="form-label">Auto-Scan Interval (days) <span class="badge bg-secondary ms-1">{{ __('Optional') }}</span></label><input type="number" class="form-control" name="auto_scan_interval"></div><div class="mb-3"><label class="form-label">Min Confidence (%) <span class="badge bg-secondary ms-1">{{ __('Optional') }}</span></label><input type="number" class="form-control" name="min_confidence"></div>
